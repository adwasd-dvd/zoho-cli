...
| OpenClaw Cliq channel | 🚧 RC-blocked on live Bot handler recheck | bot_handler_autonomy_request_ready_pending_handler_install | `cliq-channel-401..511` plus `platform-214/215/216/217/218/221` added the installable package, config/setup UX, security/employee policy gates, native outbound/inbound dispatch, Bot webhook intake, real Bot templates, Deluge Map-string hardening, direct Bot `--chat-id` fallback, quiet lifecycle/rate-limit diagnostics, no-response/handler-trigger packets, trusted reply evidence, no-publish RC artifact/install/promotion/operator handoff checks, cross-lane autonomy/operator prompts, Deluge-native `reply_mode=deluge_response`, direct-DM diagnosis, a compact Bot handler operator prompt, a no-secret template renderer for the exact Zoho Message Handler paste block, and an autonomy/operator-action request for saving that handler. Next live blocker is operator re-paste/save of the rendered Message Handler template and one fresh direct `oldsix老六` message; publish/tag/release/integrity-fill remain operator-approved only. |
