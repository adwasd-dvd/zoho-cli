...
- Implemented commands: `crm status`, `crm sdk-status`, `crm write-plan`, `crm upsert` dry-run, `crm upsert-gate`, `crm write-audit`, `crm fixture-plan`, `crm fixture-execute`, `crm fixture-evidence`, `crm modules`, `crm fields`, `crm list`, `crm get`, `crm search`, `crm users`, `crm user-get`, `crm org`, `crm profiles`, `crm profile-get`, `crm roles`, `crm role-get`, `crm layouts`, `crm coql`, `crm automation`, `crm snapshot`, `crm seed-diff`, `crm bulk-plan`, `crm notification-plan`, `crm init-plan`
- OAuth scope presets: `zoho login --with-crm` keeps the lightweight CRM default (`ZohoCRM.modules.ALL`, `ZohoCRM.settings.ALL`); `zoho login --with-storepilot-crm` adds StorePilot bootstrap scopes for users, org, bulk, notifications, and COQL. Validate with `zoho crm status --scope-profile storepilot --check-auth`. StorePilot read/query coverage now includes CRM v8 users, org details, profiles, roles, layouts, COQL, supported automation resources, snapshot export, and local-only seed diff; `crm snapshot` / `crm seed-diff` accept `--expected-org-id` or `ZOHO_ORG_ID` for production org gating, `crm snapshot` emits `snapshotSummary` coverage/count metadata, and `crm snapshot --include-automation` can include workflow rules, webhooks, automation tasks, cadences, connected workflows, and assignment thresholds in cleanup review data. `seed-diff` reports per-field `field_mapping_contracts`, missing-field `zohoType` mappings, `fields_with_property_gaps` for picklist/lookup/unique/external-id review, and readiness blockers. `crm bulk-plan` and `crm notification-plan` produce guarded dry-run contracts for future bulk jobs and webhook setup; `crm init-plan` combines seed diff, bulk, notification, automation cleanup candidates, legacy cleanup review, and a structured `manualSetupPlan` for seed manual steps, relationships/related lists, layout coverage, automation cleanup review, and notification setup into one no-write handoff. These commands do not create Zoho jobs, subscriptions, modules, fields, records, automations, or cleanup actions.
