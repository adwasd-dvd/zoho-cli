...
2. **`cliq-193` eighth slice is now delivered**: `zoho cliq app-permission-get` and `ZohoCliqClient.get_app_permission` are in place with focused tests green.
...
2. `cliq-193`: run one focused live verification pass for `app-permission-get` (`cliq status --check-auth` + `cliq app-permission-get <app_id> <permission_id>`) and archive evidence.
