...
## Next
1. `cliq-193`: ship the next smallest app-command output-hardening unit slice with focused tests, then run one fresh focused live probe (`cliq status --check-auth` + `cliq app-commands APP_PROBE_FAKE_<stamp>`) to keep blocker evidence current after the latest `20260416_173735` rerun.
