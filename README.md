...
- Current fix: `cliq-channel-524` hardens those Deluge Bot handler templates after Zoho exposed `invokeurl` output as TEXT and crashed on `webhook_response.containKey("text")`; generated Message/Mention/Participation/Context snippets now normalize `webhook_response` into `webhook_text` with KEY-VALUE `.get("text")` first and TEXT JSON `.toMap()` fallback before returning a clean `response.text` map. `cliq-channel-525` refreshed the no-publish RC evidence for that package-source change; the local RC tarball shasum is `8d49036d78c3d2c3e4e18eab88757cd74254343b` and integrity is `sha512-OebIvgCPfW0OtaSonw0Gub+DEcuaxYFL6CQp6EeEqUWejhOpO/rqfGeVs5d3/kbp7FRYZjiDL3g9DnVnRw+LSg==`. Agents still must not publish, tag, release, promote, or fill expected integrity.
