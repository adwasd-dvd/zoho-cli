...
7. **OpenClaw channel native dispatch is in place**: `cliq-channel-401` added the installable package, `cliq-channel-402` expanded config/SecretRef setup, `cliq-channel-416` added setup wizard UX, `cliq-channel-403` added secure policy gates, `cliq-channel-414` added SDK session/mention/approval seams, `cliq-channel-404` added JSON-safe `zoho` process execution, `cliq-channel-405` wired native outbound delivery, `cliq-channel-406` added fixture-backed polling normalization/dedupe over `zoho cliq chats` + `zoho cliq context`, `cliq-channel-407` added Bot webhook receive/auth/normalize intake at `/webhooks/cliq`, `cliq-channel-408` added status/read lifecycle handling, `cliq-channel-413` added turn-ledger loop prevention, `cliq-channel-409` added status/capability/routing diagnostics, `cliq-channel-410` added AI troubleshooting guidance, and `cliq-channel-417` wires accepted webhook/polling events into native OpenClaw agent turns.

## Next
1. Start `cliq-channel-415`: add observability, privacy, retention, rate-limit diagnostics, dead-letter replay guidance, and npm integrity hardening.
