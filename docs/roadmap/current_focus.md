...
2. **`cliq-191` has started with the first smallest slice**: `zoho cliq events` + `ZohoCliqClient.list_events` are now in place with targeted coverage.
...
2. `cliq-191`: add one next smallest collaboration-productivity slice (reminders or calls/meetings) with targeted tests.
