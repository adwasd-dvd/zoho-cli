...
### cliq-channel-422: RC artifact metadata promotion

- Promote source-controlled package metadata from alpha to `0.4.0-rc.1` across
  `package.json`, `openclaw.plugin.json`, runtime constants, built `dist/`, and
  package metadata tests.
- Re-run the repeatable RC pack preflight after the version promotion.
- Keep npm publishing disabled and leave `openclaw.install.expectedIntegrity`
  as the release-promotion placeholder until an actual npm/GitHub artifact is
  promoted.
- Acceptance: local/operator RC artifact evidence identifies the package as
  `0.4.0-rc.1` and has fresh shasum/integrity evidence.
- Status: complete with
  `adwasd-openclaw-zoho-cliq-0.4.0-rc.1.tgz` generated by
