...
| Operator action prompt | `ZOHO_CLI_RC_OPERATOR_ACTION_RUN_ID=20260512T223116Z-crm033-postcommit-prompt ops/scripts/zoho_cli_rc_operator_action_prompt.sh` passed locally with `status=operator_action_prompt_ready`, `nextAction=send_operator_action_prompt`, `requestIds=[save_openclaw_cliq_bot_message_handler, select_openclaw_cliq_publish_path, provide_crm_fixture_cleanup_plan, provide_crm_fixture_payload_file]`, `provide_crm_fixture_cleanup_plan.template=docs/releases/CRM_V0_5_FIXTURE_CLEANUP_PLAN_TEMPLATE.md`, `messageMarkdown` containing only placeholder recheck/follow-up commands, basename-only `reportFiles`, `actionRequestsAgentExecutableAny=false`, and no publish/tag/release/expectedIntegrity fill or Zoho write permission. Earlier JSON run `20260512T212845Z-operator-action-prompt-dev` established the wrapper contract; follow-up run `ZOHO_CLI_RC_OPERATOR_ACTION_RUN_ID=20260512T214616Z-operator-action-md-dev ops/scripts/zoho_cli_rc_operator_action_prompt.sh --md` printed the compact redacted Markdown handoff directly while still writing the JSON report. |
