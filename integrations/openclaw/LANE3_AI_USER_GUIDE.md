...
- native channel live smoke gate:
  ...
  - when a user says they sent a Bot message but no reply appears, first run
    `ops/scripts/openclaw_cliq_live_ingress_diagnostic.sh` with
    `ZOHO_CLIQ_INGRESS_LOOKBACK_SECONDS` set to the expected send window and
    ...
    diagnostic reads OpenClaw audit logs only and keeps raw webhook payloads,
    message bodies, reply bodies, and secrets out of output/report JSON.
