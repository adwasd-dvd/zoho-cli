...
| Source drift check | `OPENCLAW_CLIQ_SOURCE_DRIFT_RUN_ID=20260512T205345Z-platform215-postcommit-source ops/scripts/openclaw_cliq_rc_source_drift_check.sh` passed after the platform-215 autonomy operator-action-request commit with `status=package_source_unchanged`, no blockers, `source.headCommit=9fac452e0fd25e6a58f3054369dec01f83cb0950`, `source.repoChangedSinceManifest=true`, `source.repoChangedFileCount=38`, `packageDrift.packageChangedSinceManifest=false`, and no dirty package files. Any package file drift still blocks with `package_source_drift_detected` before operator publish. |
...
| Operator action prompt | `ZOHO_CLI_RC_OPERATOR_ACTION_RUN_ID=20260512T212845Z-operator-action-prompt-dev ops/scripts/zoho_cli_rc_operator_action_prompt.sh` passed locally with `status=operator_action_prompt_ready`, `nextAction=send_operator_action_prompt`, `requestIds=[select_openclaw_cliq_publish_path, provide_crm_fixture_cleanup_plan, provide_crm_fixture_payload_file]`, `messageMarkdown` containing only placeholder recheck commands, basename-only `reportFiles`, `actionRequestsAgentExecutableAny=false`, and no publish/tag/release/expectedIntegrity fill or Zoho write permission. Follow-up run `ZOHO_CLI_RC_OPERATOR_ACTION_RUN_ID=20260512T214616Z-operator-action-md-dev ops/scripts/zoho_cli_rc_operator_action_prompt.sh --md` printed the same compact redacted Markdown handoff directly while still writing the JSON report. |
