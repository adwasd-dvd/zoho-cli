...
2. **Latest `cliq-193` focused live rerun is complete**: reran `cliq status --check-auth` + `cliq app-installs APP_PROBE_FAKE_20260414_080256 --limit 10` and archived evidence at `tests/auto_pilot/reports/cliq193_app_installs_probe_summary_20260414_080256.json`; auth/export readiness stays healthy (`oauthReady: true`, `exportOauthReady: true`) while app-install list endpoints remain `not_supported`.
