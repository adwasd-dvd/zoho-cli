...
2. **`cliq-193` app-installs live verification is now archived**: focused probe (`cliq status --check-auth` + `cliq app-installs`) on happydistrouklimited keeps auth/export readiness healthy and classifies app-install endpoints as `not_supported`.
