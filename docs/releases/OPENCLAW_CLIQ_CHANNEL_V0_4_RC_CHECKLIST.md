...
| Source drift check | `OPENCLAW_CLIQ_SOURCE_DRIFT_RUN_ID=20260512T113043Z-operator-paths-postcommit-source ops/scripts/openclaw_cliq_rc_source_drift_check.sh` passed after the operator-path selected evidence commit with `status=package_source_unchanged`, no blockers, `headCommit=8695726a7ac4e421efd04f7e0e25a4a4df6063e3`, `headMatchesManifestSource=false`, `repoChangedSinceManifest=true`, `repoChangedFileCount=27`, `packageDrift.packageChangedSinceManifest=false`, and no dirty package files. Any package file drift still blocks with `package_source_drift_detected` before operator publish. |
