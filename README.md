...
2. **Authenticate**
   ...
   # For StorePilot CRM bootstrap scopes:
   zoho login --with-storepilot-crm
   ```

3. **Verify CRM scope readiness**
   ```bash
   zoho auth status
...
- Current slice: `cliq-channel-523` is complete after `cliq-channel-506/507/508/509/510/511/512/513/514/515/516/517/518/519/520/521/522`; the channel supports `reply_mode=deluge_response` for real Bot handlers, captures the native OpenClaw final answer into webhook JSON `text`, and Message/Mention/Participation/Context Deluge templates now copy only `webhook_response.text` into a clean `response.text` map before returning it. This keeps Zoho's native Bot response surface small enough to render while avoiding the second-hop OAuth `zoho cliq send` path. The no-response diagnostics expose `diagnosis.code` plus `handlers.directMessageRequirement` so public-callback-green/no-ingress cases point at Zoho handler save/trigger state, especially a missing saved **Message Handler** for plain direct Bot DMs. `ops/scripts/openclaw_cliq_bot_handler_operator_prompt.sh --md` renders the compact human handoff for that Zoho UI step, and `ops/scripts/openclaw_cliq_bot_handler_template_render.sh --md --handlers message` renders a minimal direct-DM Deluge block that uses only `message`, `user`, and `chat`, avoids optional Deluge variables that can be undefined before `invokeurl`, closes assigned `invokeurl [...]` tasks with `];`, and returns the clean response map. The cross-lane RC autonomy packet no longer surfaces the stale Message Handler save request by default after the operator-confirmed direct Bot fix; forced/debug triage remains available with `ZOHO_CLI_RC_AUTONOMY_INCLUDE_BOT_HANDLER_REQUEST=force`. Bot emoji support is deliberately conservative: true Zoho lifecycle/status reactions are attempted only when an inbound event has a native message id plus chat/channel route facts; synthetic Deluge ids such as `zoho-message-*` and `webhook-*` skip true reaction/read calls with `synthetic_message_id`, and Deluge-native synthetic replies get a tested visible `✅ ` prefix fallback with privacy-safe `reactionFallback` diagnostics. The source-drift guard skips newer blocked handoff drafts and defaults to the newest ready handoff manifest, keeping recurring autonomy checks from false-blocking on stale failed manifest attempts. The no-publish package chain was refreshed after the reaction-fallback package-source change; the local RC tarball shasum is `a41406448dda8ca6c7229750ff14cb535c7570e5` and integrity is `sha512-5CxFO7kA7VX6ztyNiMzejdk1VC7mLjxZ2fBtZ2mLNdRr64Ci8reP932YJr+pPg6SDJpsjYI734ETZJB9FYaciQ==`. The existing RC chain still includes the `cliq-channel-473/474` no-response handler-trigger flow, `cliq-channel-501/502/503` direct chat-id fallback plus quiet lifecycle/rate-limit diagnostics and artifact refresh, cross-lane autonomy/operator prompt tooling, read-only publish handoff, local artifact/install/promotion checks, and all publish/tag/release/integrity-fill actions remain operator-approved only.
- Current fix: `cliq-channel-524` hardened those Deluge Bot handler templates after Zoho exposed `invokeurl` output as TEXT and crashed on `webhook_response.containKey("text")`; `cliq-channel-526` follows the latest no-visible-reply evidence where OpenClaw ingress/model/dispatch completed but Zoho did not render the returned Bot text. Generated Message/Mention/Participation/Context snippets now normalize `webhook_response` into `webhook_text` with Deluge `getJSON("text")` first, with a KEY-VALUE `.get("text")` fallback before returning a clean `response.text` map, and no longer depend on `.toString().toMap()`. `cliq-channel-527` refreshed the no-publish RC evidence for that package-source change; the local RC tarball shasum is `789329438bcd1b4d60ca3970f702bbc3ce192f27` and integrity is `sha512-BB+zg/d3waTGAs8O06ZtC2ACB0wkoVGTwWbfd87TDeXUctQBKTocn89avvZtcBIgZHAmc0c4eRx2de4cL/rpMw==`. Agents still must not publish, tag, release, promote, or fill expected integrity.
...
- Next operator-gated slice: `cliq-channel-496` waits for the native Cliq RC publish-path selection (`local_operator_rc`, `npm_rc_publish`, or `github_release_artifact`). Agents may prepare/read handoffs only; publish, tag, npm promotion, GitHub release, and expectedIntegrity fill remain operator-only. True Bot reaction support remains gated on Zoho providing a native inbound message id; synthetic Message Handler ids use the documented emoji-prefixed reply fallback.
...
`zoho-cli` uses file-backed token storage by default. This avoids macOS
Keychain confirmation dialogs in non-interactive agents and gateway processes.

```bash
export ZOHO_CONFIG=/path/to/zoho-config.json
export ZOHO_TOKEN_PASSWORD='local-only-passphrase'
zoho login --account bot@example.com --with-cliq --with-storepilot-crm
```

To opt back into the legacy Keychain behavior for a human terminal session:

```bash
export ZOHO_TOKEN_BACKEND=keychain
```

If an existing token only lives in Keychain, either run one explicit Keychain
session to migrate/re-login, or just re-run `zoho login` with the default file
backend.
