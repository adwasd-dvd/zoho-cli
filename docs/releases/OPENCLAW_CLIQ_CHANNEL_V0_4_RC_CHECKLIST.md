...
| Source drift check | `OPENCLAW_CLIQ_SOURCE_DRIFT_RUN_ID=20260512T132144Z-crm-readiness-action-boundary-postcommit-source ops/scripts/openclaw_cliq_rc_source_drift_check.sh` passed after the CRM readiness-action-boundary commit with `status=package_source_unchanged`, no blockers, `headCommit=25eb507d75c6100aad6ca7e7ddd418a60815c87e`, `headMatchesManifestSource=false`, `repoChangedSinceManifest=true`, `repoChangedFileCount=28`, `packageDrift.packageChangedSinceManifest=false`, and no dirty package files. Any package file drift still blocks with `package_source_drift_detected` before operator publish. |
