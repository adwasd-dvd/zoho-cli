# Current Focus

## Immediate
1. **`cliq-165` remains the top blocker**: maintenance export scopes are granted, but both `cliq export-chats` list and `--chat-id` still return API-side `inactive_appaccount_user` until Cliq admin-side app-account activation is completed.
2. **`cliq-193` response-envelope output hardening is complete**: `cliq apps` + `cliq app-get` now unwrap top-level `response`/`result` wrappers (including nested `data.records.record.item`) with focused unit coverage (`tests/test_cli.py::test_cliq_apps_accepts_top_level_response_wrapper_shape`, `tests/test_cli.py::test_cliq_app_get_accepts_top_level_response_wrapper_shape`).
3. **Next step is one focused live `cliq-193` rerun** (`cliq status --check-auth` + `cliq app-get`) to archive post-hardening evidence.

## Next
1. `cliq-193`: run one focused live pass (`cliq status --check-auth` + `cliq app-get`) and archive evidence for the response-envelope hardening slice.
2. `cliq-193`: ship the next smallest app-governance output-hardening increment with focused unit coverage.
3. `cliq-165`: rerun export verification immediately after app-account activation (`list` + one `--chat-id` export) and archive first success JSON.
4. `crm-002`: remain blocked until CRM-enabled org access is available.
