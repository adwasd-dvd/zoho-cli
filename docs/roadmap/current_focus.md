...
2. **`cliq-193` focused live verification for `app-install-get` is now archived**: `cliq status --check-auth` stayed healthy (`oauthReady: true`, `exportOauthReady: true`) while `cliq app-install-get` classified app-install detail endpoints as `not_supported` on `happydistrouklimited`.
