...
| Source drift check | `OPENCLAW_CLIQ_SOURCE_DRIFT_RUN_ID=20260512T143844Z-crm-next-command-preview-postcommit-source ops/scripts/openclaw_cliq_rc_source_drift_check.sh` passed after the CRM next-command-preview commit with `status=package_source_unchanged`, no blockers, `headCommit=3f1773c7d5deb7e1dae68db84445017d0efd0449`, `headMatchesManifestSource=false`, `repoChangedSinceManifest=true`, `repoChangedFileCount=29`, `packageDrift.packageChangedSinceManifest=false`, and no dirty package files. Any package file drift still blocks with `package_source_drift_detected` before operator publish. |
