...
2. **`cliq-193` seventh app-governance slice is now in code**: `app-install-get` command/client coverage is in place with focused tests and is ready for one focused live verification pass.
...
2. `cliq-193`: run one focused live verification pass for `cliq app-install-get` and archive evidence.
