...
- Live smoke harness: `ops/scripts/crm_fixture_live_smoke.sh` runs the controlled fixture sequence and writes redacted report files; it defaults to dry-run and only executes live when `ZOHO_CRM_FIXTURE_EXECUTE=1` and `ZOHO_CRM_ALLOW_LIVE_FIXTURE=1`.
...
- Next active target: operator-approved CRM fixture live evidence while public native Cliq Bot callback verification waits on a reachable tunnel/gateway URL.
