...
2. **`cliq-193` focused live verification rerun is archived**: `cliq status --check-auth` stays healthy (`oauthReady: true`, `exportOauthReady: true`), while both `cliq apps` and `cliq app-get` still classify as `not_supported` on happydistrouklimited (evidence: `tests/auto_pilot/reports/cliq193_probe_summary_20260413_030256.json`).
