...
3. **Focused live evidence is refreshed**: `cliq status --check-auth` + `cliq app-commands APP_PROBE_FAKE_20260415_083526` keeps auth/export readiness healthy (`oauthReady: true`, `exportOauthReady: true`) while app-command list endpoints remain `not_supported` (evidence: `tests/auto_pilot/reports/cliq193_app_commands_probe_summary_20260415_083526.json`).
