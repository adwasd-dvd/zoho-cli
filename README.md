...
| OpenClaw Cliq channel | 🚧 RC-blocked on live Bot handler recheck | bot_handler_autonomy_prompt_prioritized_pending_handler_install | `cliq-channel-401..512` plus `platform-214/215/216/217/218/221` added the installable package, config/setup UX, security/employee policy gates, native outbound/inbound dispatch, Bot webhook intake, real Bot templates, Deluge Map-string hardening, direct Bot `--chat-id` fallback, quiet lifecycle/rate-limit diagnostics, no-response/handler-trigger packets, trusted reply evidence, no-publish RC artifact/install/promotion/operator handoff checks, cross-lane autonomy/operator prompts, Deluge-native `reply_mode=deluge_response`, direct-DM diagnosis, a compact Bot handler operator prompt, a no-secret template renderer for the exact Zoho Message Handler paste block, and a first-position autonomy/operator-action request for saving that handler. Next live blocker is operator re-paste/save of the rendered Message Handler template and one fresh direct `oldsix老六` message; publish/tag/release/integrity-fill remain operator-approved only. |
...
- Next active target: save/recheck the real `oldsix老六` Message Handler before operator decision on the native Zoho Cliq channel RC publish/tag path; `ops/scripts/zoho_cli_rc_operator_action_prompt.sh --md` now includes `nextOperatorActionId=save_openclaw_cliq_bot_message_handler`, fresh Bot triage starts with `ops/scripts/openclaw_cliq_bot_handler_operator_prompt.sh --md`, then `ops/scripts/openclaw_cliq_bot_handler_template_render.sh --md --handlers message` for the exact paste block before editing Zoho, and `ops/scripts/openclaw_cliq_bot_no_response_packet.sh` after one fresh message. CRM fixture live run remains gated on a dedicated payload and cleanup plan.
