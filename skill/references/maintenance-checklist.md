...
4. Keep `skill/references/github-intake-workflow.md` aligned with the canonical GitHub repository, existing labels, and issue templates.
5. Verify help surface still contains required loop/status commands:
   ```bash
...
6. Run focused validation:
   ...
   ```
7. For RC updates, verify `zoho --version` matches the approved RC version before syncing AI-user skill files.
8. For Cliq status/capability maintenance, inspect `zoho_cli/commands/cliq_readiness.py` first; for `whoami` / `user-resolve`, inspect `zoho_cli/commands/cliq_identity.py`; for `users` / `teams`, inspect `zoho_cli/commands/cliq_org_directory.py`; for org-admin list commands, inspect `zoho_cli/commands/cliq_org_admin.py`; for events/reminders/meetings/databases, inspect `zoho_cli/commands/cliq_productivity.py`; for widgets/map-tickers/custom-domains/custom-emails, inspect `zoho_cli/commands/cliq_platform_extensions.py`; for channel/member/chat-control commands, inspect `zoho_cli/commands/cliq_channel_management.py`.
9. For v0.4 native OpenClaw Cliq channel work, keep `docs/architecture/OPENCLAW_CLIQ_CHANNEL_0_4_PLAN.md`, `integrations/openclaw/CLIQ_CHANNEL_DEVELOPMENT_GUIDE.md`, and `skill/references/openclaw-cliq-channel.md` aligned with current OpenClaw plugin APIs.
10. Update changelog and state files for traceability.
