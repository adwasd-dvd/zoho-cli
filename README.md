...
- Current slice: `cliq-channel-509` is complete after `cliq-channel-506/507/508`; the channel supports `reply_mode=deluge_response` for real Bot handlers, captures the native OpenClaw final answer into webhook JSON `text`, updates Message/Mention/Participation/Context Deluge templates to `return webhook_response` when that `text` key is present, and the no-response diagnostics now expose `diagnosis.code` plus `handlers.directMessageRequirement` so public-callback-green/no-ingress cases point at Zoho handler save/trigger state, especially a missing saved **Message Handler** for plain direct Bot DMs. `ops/scripts/openclaw_cliq_bot_handler_operator_prompt.sh --md` now renders the compact human handoff for that Zoho UI step. The source-drift guard skips newer blocked handoff drafts and defaults to the newest ready handoff manifest, keeping recurring autonomy checks from false-blocking on stale failed manifest attempts. The no-publish package chain was refreshed after the package-local doc change; the local RC tarball shasum is `10a5eb6cafb3343bcec8253663745d9e883e81b8`. This keeps direct Bot replies on Zoho's native handler response path and avoids the second-hop OAuth send that can be rate-limited. The existing RC chain still includes the `cliq-channel-473/474` no-response handler-trigger flow, `cliq-channel-501/502/503` direct chat-id fallback plus quiet lifecycle/rate-limit diagnostics and artifact refresh, cross-lane autonomy/operator prompt tooling, read-only publish handoff, local artifact/install/promotion checks, and all publish/tag/release/integrity-fill actions remain operator-approved only.
...
| OpenClaw Cliq channel | 🚧 RC-blocked on live Bot handler recheck | bot_handler_operator_prompt_ready_pending_handler_install | `cliq-channel-401..509` plus `platform-214/215/216/217/218/221` added the installable package, config/setup UX, security/employee policy gates, native outbound/inbound dispatch, Bot webhook intake, real Bot templates, Deluge Map-string hardening, direct Bot `--chat-id` fallback, quiet lifecycle/rate-limit diagnostics, no-response/handler-trigger packets, trusted reply evidence, no-publish RC artifact/install/promotion/operator handoff checks, cross-lane autonomy/operator prompts, Deluge-native `reply_mode=deluge_response`, direct-DM diagnosis, and a compact Bot handler operator prompt. Next live blocker is operator re-paste/save of the current Message Handler template and one fresh direct `oldsix老六` message; publish/tag/release/integrity-fill remain operator-approved only. |
...
- Next active target: operator decision on the native Zoho Cliq channel RC publish/tag path now that local promotion preflight reports `ready_for_operator_publish`; fresh Bot no-response triage starts with `ops/scripts/openclaw_cliq_bot_handler_operator_prompt.sh --md` before editing Zoho and `ops/scripts/openclaw_cliq_bot_no_response_packet.sh` after one fresh message; CRM fixture live run remains gated on a dedicated payload and cleanup plan.
