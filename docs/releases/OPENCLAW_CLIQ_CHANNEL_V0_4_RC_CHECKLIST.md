...
| Source drift check | `OPENCLAW_CLIQ_SOURCE_DRIFT_RUN_ID=20260512T124844Z-crm-action-boundary-postcommit-source ops/scripts/openclaw_cliq_rc_source_drift_check.sh` passed after the CRM operator-action-boundary commit with `status=package_source_unchanged`, no blockers, `headCommit=8a67bbb36b551a47ff61b898d85d68a6a92fa42f`, `headMatchesManifestSource=false`, `repoChangedSinceManifest=true`, `repoChangedFileCount=28`, `packageDrift.packageChangedSinceManifest=false`, and no dirty package files. Any package file drift still blocks with `package_source_drift_detected` before operator publish. |
