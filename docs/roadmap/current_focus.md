...
2. **Latest `cliq-193` focused live rerun remains externally blocked**: `cliq status --check-auth` + `cliq app-commands APP_PROBE_FAKE_20260415_162710` kept auth/export readiness healthy (`oauthReady: true`, `exportOauthReady: true`) while app-command list endpoints remained `not_supported` (evidence: `tests/auto_pilot/reports/cliq193_app_commands_probe_summary_20260415_162710.json`).
