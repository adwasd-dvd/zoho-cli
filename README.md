...
- Implemented commands: `crm status`, `crm sdk-status`, `crm write-plan`, `crm upsert` dry-run, `crm upsert-gate`, `crm write-audit`, `crm fixture-plan`, `crm fixture-execute`, `crm fixture-evidence`, `crm modules`, `crm fields`, `crm list`, `crm get`, `crm search`, `crm users`, `crm user-get`, `crm org`, `crm coql`
- OAuth scope presets: `zoho login --with-crm` keeps the lightweight CRM default (`ZohoCRM.modules.ALL`, `ZohoCRM.settings.ALL`); `zoho login --with-storepilot-crm` adds StorePilot bootstrap scopes for users, org, bulk, notifications, and COQL. Validate with `zoho crm status --scope-profile storepilot --check-auth`. StorePilot read/query coverage now includes CRM v8 users, org details, and COQL; bulk and notifications are auth-ready follow-on surfaces, not live-write shortcuts.
