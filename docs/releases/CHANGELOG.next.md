...
- `zoho login --no-browser` now supports safer manual auth URL selection: when no redirect is configured it prints both legacy hosted (`https://example.com/zoho/oauth/callback`) and localhost (`http://localhost:{port}/callback`) auth URLs, accepts pasted query-string/bare code input, and auto-derives the redirect URI from the pasted full callback URL for token exchange parity.
