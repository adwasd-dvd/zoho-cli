...
import type {
  CliqNativeDispatchResult,
  CliqNativeReplyTransport,
} from "./native-dispatch.js";
import {
  buildCliqAuditEvent,
  CLIQ_WEBHOOK_BODY_MAX_BYTES,
  CLIQ_WEBHOOK_BODY_TIMEOUT_MS,
  CLIQ_WEBHOOK_MAX_IN_FLIGHT_PER_KEY,
  CLIQ_WEBHOOK_MAX_TRACKED_KEYS,
  CLIQ_WEBHOOK_RATE_LIMIT_MAX_REQUESTS,
  CLIQ_WEBHOOK_RATE_LIMIT_WINDOW_MS,
  emitCliqAuditEvent,
...
function splitDelugeEntries(value: string): string[] {
...
function findDelugeEntrySeparator(value: string): number {
  let depth = 0;
  let quote: string | undefined;
  let escaped = false;
  for (let index = 0; index < value.length; index += 1) {
...
function stripDelugeQuotes(value: string): string {
  const trimmed = value.trim();
...
function parseDelugeListString(value: string): unknown[] | undefined {
  const trimmed = value.trim();
...
function parseDelugeScalar(value: string): unknown {
  const trimmed = value.trim();
  if (!trimmed) return "";
  if (
    trimmed.startsWith("{") ||
...
function parseDelugeMapString(
  ...
  const inner = trimmed.slice(1, -1).trim();
  if (!inner) return {};
  if (findDelugeEntrySeparator(inner) < 0) return undefined;
...
export function parseCliqWebhookPayload(
  ...
  if (parsedDelugeMap) return parsedDelugeMap;
...
function normalizeReplyMode(raw: string | undefined): CliqNativeReplyTransport | undefined {
  const normalized = (raw ?? "")
    .trim()
    .toLowerCase()
    .replace(/[\s-]+/g, "_");
...
export function shouldProcessCliqWebhookPayloadInBackground(
  payload: unknown,
): boolean {
  const envelope = extractCliqWebhookEnvelope(payload);
  return (
    envelope?.replyMode === "zoho_cli" && shouldAcceptHandler(envelope.handlerKind)
  );
}
...
function isCliqNativeDispatchResult(
  value: unknown,
): value is CliqNativeDispatchResult {
  return value !== null && typeof value === "object" && "deliveryTransport" in value;
}

function responseNativeDispatchSummary(
  value: unknown,
): Record<string, unknown> | undefined {
  if (!isCliqNativeDispatchResult(value)) return undefined;
  return {
...
function responseReplyText(value: unknown): string | undefined {
  if (!isCliqNativeDispatchResult(value)) return undefined;
  if (value.deliveryTransport !== "deluge_response") return undefined;
      ...
      if (shouldProcessCliqWebhookPayloadInBackground(payload)) {
        ...
        void processCliqWebhookPayload({
          ...
          options.logger?.error?.(
            `[zoho-cliq] background webhook handler failed: ${
      ...
      handler: createCliqWebhookHttpHandler({
        ...
        onEvent: async (event, context) => {
          return nativeDispatcher(event, {
            ...
            source: "webhook",
            replyTransport:
              context.replyMode === "deluge_response"
                ? "deluge_response"
                : "zoho_cli",
          });
        },
