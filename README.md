...
- Agent binding: `cliq-channel-520` verifies `oldsix老六` / `cliq/default` is bound to OpenClaw agent `zoho-employee-test` through OpenClaw `bindings[]`, matching Discord's account-to-agent selection model. Deep Cliq channel status now exposes `agentBinding.agentId` plus `diagnostics.agentBinding.agentId` before live Bot traffic.
- Next Bot product slice: `cliq-channel-519` tracks emoji reaction support for Deluge-native direct Bot messages. The implementation must first determine whether Zoho's Bot Message Handler exposes a real inbound message id; true `zoho cliq react` / `zoho cliq status-react` is allowed only with that id, otherwise the channel should use a documented/tested fallback such as emoji-prefixed Bot replies.
