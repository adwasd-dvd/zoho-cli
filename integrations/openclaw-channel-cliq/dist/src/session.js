import { buildChannelOutboundSessionRoute, buildThreadAwareOutboundSessionRoute, stripChannelTargetPrefix, } from "openclaw/plugin-sdk/channel-core";
import { CLIQ_CHANNEL_ID, CLIQ_PLUGIN_ID } from "./constants.js";
const DEFAULT_CLIQ_NETWORK = "default";
function normalizeNonEmpty(raw) {
    return String(raw ?? "").trim();
}
function stripCliqProviderPrefix(raw) {
    return stripChannelTargetPrefix(raw.trim(), "cliq", "zoho-cliq", "zoho");
}
function splitThreadSuffix(raw) {
    const marker = ":thread:";
...
export function normalizeCliqSessionToken(raw) {
    const trimmed = normalizeNonEmpty(raw);
    if (!trimmed)
        return "unknown";
    return encodeURIComponent(trimmed).toLowerCase();
}
function decodeCliqSessionToken(raw) {
    try {
...
function targetFromNative(chatType, nativeId, threadId) {
...
function chatTypeFromPreferredKind(preferredKind) {
...
export function parseCliqExplicitTarget(raw, preferredKind) {
    ...
    if (atMatch?.[1])
        return targetFromNative("direct", atMatch[1], threadId);
    const kindMatch = /^(user|dm|direct|channel|group|chat|conversation|room):(.+)$/iu.exec(target);
...
export function normalizeCliqTarget(raw) {
    return parseCliqExplicitTarget(raw)?.to;
}
export function inferCliqTargetChatType(raw) {
    return parseCliqExplicitTarget(raw)?.chatType ?? "channel";
}
export function buildCliqSessionPeerId(peer) {
    return [
        "account",
...
function parseCliqSessionPeerId(rawId) {
    const { base, threadId } = splitThreadSuffix(rawId);
    const parts = base.split(":");
    if (parts.length !== 6)
        return null;
    const [accountLabel, accountId, networkLabel, network, chatType, nativeId] = parts;
...
export function buildCliqOutboundSessionRoute(params) {
    const parsed = parseCliqExplicitTarget(params.resolvedTarget?.to ?? params.target, params.resolvedTarget?.kind) ??
    ...
    if (!explicitThreadId && !params.replyToId)
        return route;
