...
- Next automation slice: `cliq-channel-523` retires the stale `save_openclaw_cliq_bot_message_handler` ask from autonomy/operator prompts after the operator-confirmed direct Bot fix, while preserving a forced/debug path for future handler-save triage. True Bot reaction support remains gated on Zoho providing a native inbound message id; synthetic Message Handler ids use the documented emoji-prefixed reply fallback.
