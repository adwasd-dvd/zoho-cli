...
- Cliq maintenance export scope handling now also treats API code `oauth_scope_invalid` as export-scope miss (in addition to `oauthtoken_scope_invalid`), and both export paths now point re-auth guidance to `ZohoCliq.Org.Admin` so CLI hints match the live blocker.
