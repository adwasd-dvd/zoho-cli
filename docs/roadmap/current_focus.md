...
## Next
1. `cliq-193`: run one matching focused live rerun (`cliq status --check-auth` + `cliq app-commands APP_PROBE_FAKE_<stamp>`) and archive evidence for the new PascalCase wrapper hardening increment.
2. `cliq-193`: implement the next smallest app-command output-hardening increment with focused tests.
