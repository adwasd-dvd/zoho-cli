...
8. **CRM v0.5 SDK adoption is in parity prep**: `crm-003` adds `zoho crm sdk-status`, optional `zoho-cli[crm-sdk]` packaging for official `zohocrmsdk8_0==5.0.0`, and `docs/architecture/CRM_V0_5_SDK_ADOPTION_PLAN.md`; `crm-004` adds `zoho_cli/crm_sdk.py` with data-center mapping, `ZOHO_CRM_SDK_RESOURCE_PATH` cache/resource policy, and JSON-safe read-only adapter skeleton. Current CRM commands still use the HTTP adapter by default.

## Next
1. Start `crm-005` read-only SDK parity behind an explicit adapter gate while public native Cliq Bot callback verification waits on a reachable tunnel/gateway URL.
