...
import {
  ...
  patchCliqAccountConfig,
  resolveCliqAccount,
  setCliqAccountEnabled,
  type CliqResolvedAccount,
} from "./config.js";
import {
  CLIQ_CHANNEL_ID,
  DEFAULT_ZOHO_CLI,
  OPENCLAW_PLUGIN_API_RANGE,
} from "./constants.js";

type CliqDmPolicy = "pairing" | "allowlist" | "open" | "disabled";
...
function normalizeCliqDmPolicy(value: unknown): CliqDmPolicy {
...
function splitAllowFrom(raw: string): string[] {
  return raw
    .split(/[\s,]+/u)
...
function normalizeAllowFromId(raw: string): string | null {
  const value = raw.trim();
  if (!value) return null;
  if (value === "*") return value;
  if (value.startsWith("user:")) return value.slice("user:".length) || null;
  if (value.startsWith("@")) return value.slice(1) || null;
  return /^[A-Za-z0-9_.:@-]+$/u.test(value) ? value : null;
}

function describeAuthState(account: CliqResolvedAccount): string {
...
function hasEmployeeScope(account: CliqResolvedAccount): boolean {
  if (account.employeeMode.enabled === false) return true;
...
export function resolveCliqSetupStateCodes(params: {
  ...
  const states: CliqSetupStateCode[] = [];
  ...
  if (
    ((normalizeCliqDmPolicy(account.dmPolicy) === "allowlist" &&
...
export const cliqSetupWizard: ChannelSetupWizard = {
  ...
  status: {
    ...
    configuredHint:
      "Run zoho cliq status --check-auth before enabling live traffic.",
    unconfiguredHint:
      "Configure account, network, zoho-cli auth, webhook/polling choice, and allowlist.",
    configuredScore: 80,
    unconfiguredScore: 20,
    resolveConfigured: ({ cfg, accountId }) =>
      isCliqAccountConfigured(resolveCliqAccount(cfg, accountId)),
  ...
  envShortcut: {
    prompt:
      "Use ZOHO_ACCOUNT, ZOHO_CONFIG, ZOHO_TOKEN_PASSWORD, and ZOHO_CLIQ_WEBHOOK_SECRET from the environment.",
    preferredEnvVar: "ZOHO_TOKEN_PASSWORD",
    isAvailable: () =>
      Boolean(
        process.env.ZOHO_ACCOUNT ||
  ...
  resolveShouldPromptAccountIds: ({ cfg, shouldPromptAccountIds }) =>
    shouldPromptAccountIds || defaultCliqAccountId(cfg) !== "default",
  ...
  textInputs: [
    {
      ...
      currentValue: ({ cfg, accountId }) =>
        resolveCliqAccount(cfg, accountId).name,
      applySet: ({ cfg, accountId, value }) =>
        applyTextPatch(cfg, accountId, { name: value.trim() }),
    },
    {
      inputKey: "userId",
      ...
      validate: ({ value }) =>
        value.trim().includes("@") ? undefined : "Enter a Zoho account email.",
      normalizeValue: ({ value }) => value.trim(),
      applySet: ({ cfg, accountId, value }) =>
        applyTextPatch(cfg, accountId, { accountEmail: value }),
    },
    {
      inputKey: "region",
      ...
      required: true,
      currentValue: ({ cfg, accountId }) =>
        resolveCliqAccount(cfg, accountId).network,
      shouldPrompt: ({ currentValue }) => !currentValue,
      validate: ({ value }) =>
        value.trim().length > 0 ? undefined : "Enter a Cliq network.",
      normalizeValue: ({ value }) => value.trim(),
      applySet: ({ cfg, accountId, value }) =>
        applyTextPatch(cfg, accountId, { network: value }),
    },
    {
      inputKey: "cliPath",
      ...
      placeholder: "zoho",
      currentValue: ({ cfg, accountId }) =>
        resolveCliqAccount(cfg, accountId).cliPath || DEFAULT_ZOHO_CLI,
      normalizeValue: ({ value }) => value.trim() || DEFAULT_ZOHO_CLI,
      applySet: ({ cfg, accountId, value }) =>
        applyTextPatch(cfg, accountId, { cliPath: value }),
    },
    {
      inputKey: "tokenFile",
      ...
      normalizeValue: ({ value }) => value.trim(),
      applySet: ({ cfg, accountId, value }) =>
        applyTextPatch(cfg, accountId, { configPath: value }),
    },
    {
      inputKey: "audience",
      ...
      normalizeValue: ({ value }) => value.trim(),
      applySet: ({ cfg, accountId, value }) =>
        applyTextPatch(cfg, accountId, { defaultTo: value }),
    },
  ],
  dmPolicy: {
    ...
    getCurrent: (cfg, accountId) =>
      normalizeCliqDmPolicy(resolveCliqAccount(cfg, accountId).dmPolicy),
  ...
  allowFrom: {
    ...
    invalidWithoutCredentialNote:
      "Add at least one trusted Cliq user id before live testing.",
    parseInputs: splitAllowFrom,
    parseId: normalizeAllowFromId,
    resolveEntries: async ({ entries }) =>
      entries.map<ChannelSetupWizardAllowFromEntry>((entry) => {
