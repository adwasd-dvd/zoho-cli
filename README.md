...
| OpenClaw Cliq channel | 🚧 RC-blocked on live Bot handler recheck | deluge_native_reply_ready_pending_handler_install | `cliq-channel-401..504` plus `platform-214/215/216/217/218` added the installable package, config/setup UX, security/employee policy gates, native outbound/inbound dispatch, Bot webhook intake, real Bot templates, Deluge Map-string hardening, direct Bot `--chat-id` fallback, quiet lifecycle/rate-limit diagnostics, no-response/handler-trigger packets, trusted reply evidence, no-publish RC artifact/install/promotion/operator handoff checks, cross-lane autonomy/operator prompts, and now Deluge-native `reply_mode=deluge_response` so real Bot handlers can return the OpenClaw final answer as webhook response `text` without a second OAuth send. Next live blocker is operator re-paste/save of the current Message Handler template and one fresh direct `oldsix老六` message; publish/tag/release/integrity-fill remain operator-approved only. |
