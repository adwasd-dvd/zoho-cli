...
CRM_FIXTURE_AGENT_NEXT_COMMAND_SCRIPT = (
...
def _write_zoho_cli_rc_autonomy_cliq_packet(
    tmp_path: Path,
    *,
    status: str = "awaiting_operator_publish_path",
) -> Path:
    packet = tmp_path / f"cliq-{status}.json"
    packet.write_text(
        json.dumps(
            {
                ...
                "blockers": (
                    ...
                    if status == "awaiting_operator_publish_path"
                    else []
                ),
                "nextAction": (
                    "operator_select_publish_path"
                    if status == "awaiting_operator_publish_path"
                    else "operator_review_selected_publish_path"
                ),
                "selectedPublishPath": (
                    "local_operator_rc"
                    if status == "operator_publish_selection_ready"
                    else None
                ),
                "reportsReady": {
                    "publishPlan": True,
                ...
                "verifiedStatuses": {
                    "publishPlan": "operator_publish_plan_ready",
                    "sourceDrift": "package_source_unchanged",
                    "selectionReview": (
...
def _write_zoho_cli_rc_autonomy_crm_packet(
    tmp_path: Path,
    *,
    status: str = "operator_input_required",
) -> Path:
    next_agent_command = None
    if status in {"operator_input_required", "agent_next_command_ready"}:
        next_agent_command = {
            "id": (
            ...
            "command": (
                "ZOHO_CRM_FIXTURE_PAYLOAD_FILE=<payload> "
                "ops/scripts/crm_fixture_operator_packet.sh"
                if status == "operator_input_required"
                else "ZOHO_CRM_FIXTURE_EXECUTE=0 ops/scripts/crm_fixture_live_smoke.sh"
            ),
            "agentMayExecute": status == "agent_next_command_ready",
            "dryRunOnly": True,
            "operatorOnly": False,
            "writesZohoData": False,
            "requiresExplicitOperatorApproval": False,
            "requiresOperatorInput": status == "operator_input_required",
        }

    packet = tmp_path / f"crm-{status}.json"
    packet.write_text(
        json.dumps(
            {
                ...
                "status": status,
                "nextAction": (
                    "collect_operator_input_for_next_agent_command"
                    if status == "operator_input_required"
                    else "execute_next_agent_command"
                    if status == "agent_next_command_ready"
                    else "inspect_crm_fixture_operator_packet"
                ),
                "packet": {
                    "status": "blocked"
                    if status == "operator_input_required"
                    else "payload_preflight_ready",
                    "blockers": [],
                    "reportFiles": {},
                    "reportsReady": {},
                },
                "operatorReview": {
                    ...
                    "nextAgentExecutableCommandId": (
                        next_agent_command["id"] if next_agent_command else None
                    ),
                    "nextAgentCommand": next_agent_command,
                    "agentExecutableCommandAllowed": next_agent_command is not None,
                    "agentMayExecuteNextCommand": (
                        status == "agent_next_command_ready"
                    ),
                    "agentMayExecuteAfterOperatorInput": (
                ...
                "safety": {
                    ...
                    "nextCommandAllowedForAgent": next_agent_command is not None,
                    "nextCommandDryRunOnly": next_agent_command is not None,
                    "nextCommandWritesZohoData": False,
                    "nextCommandRequiresExplicitOperatorApproval": False,
                },
            }
        ),
        encoding="utf-8",
...
def test_zoho_cli_rc_autonomy_packet_collects_operator_input(
    ...
    result = subprocess.run(
        ...
        env={
            **os.environ,
            "ZOHO_CLI_RC_AUTONOMY_RUN_ID": "unit-autonomy-input",
    ...
    assert payload["safety"]["agentMayExecuteCliqSelectedPath"] is False
    assert payload["reportFiles"] == {
        "autonomyPacket": "autonomy.json",
        "cliqDecisionPacket": cliq_packet.name,
        "crmAgentNextCommand": crm_packet.name,
    }
    assert payload["commandsRan"] == {
        "cliqDecisionPacket": False,
        "crmAgentNextCommand": False,
    }
    assert json.loads(packet_file.read_text()) == payload
...
def test_zoho_cli_rc_autonomy_packet_surfaces_safe_crm_agent_command(
    ...
    cliq_packet = _write_zoho_cli_rc_autonomy_cliq_packet(tmp_path)
    crm_packet = _write_zoho_cli_rc_autonomy_crm_packet(
        tmp_path,
        status="agent_next_command_ready",
    )
    ...
    result = subprocess.run(
        ...
        env={
            **os.environ,
            "ZOHO_CLI_RC_AUTONOMY_RUN_ID": "unit-autonomy-ready",
            "ZOHO_CLI_RC_AUTONOMY_REPORT_DIR": str(tmp_path),
    ...
    assert result.returncode == 0, output
    payload = json.loads(result.stdout)
    assert payload["status"] == "agent_next_command_ready"
    assert payload["nextAction"] == "execute_crm_next_agent_command"
    assert payload["recommendedAgentCommand"]["id"] == (
        "run_crm_fixture_live_smoke_dry_run"
    )
    assert payload["safety"]["agentMayRunCrmNextCommand"] is True
...
def test_zoho_cli_rc_autonomy_packet_stops_for_selected_publish_path(
    ...
    cliq_packet = _write_zoho_cli_rc_autonomy_cliq_packet(
        tmp_path,
        status="operator_publish_selection_ready",
    )
    crm_packet = _write_zoho_cli_rc_autonomy_crm_packet(
        tmp_path,
    ...
    result = subprocess.run(
        ...
        env={
            **os.environ,
            "ZOHO_CLI_RC_AUTONOMY_RUN_ID": "unit-autonomy-stop",
            "ZOHO_CLI_RC_AUTONOMY_REPORT_DIR": str(tmp_path),
            "ZOHO_CLI_RC_AUTONOMY_FILE": str(tmp_path / "autonomy.json"),
            "ZOHO_CLI_RC_AUTONOMY_CLIQ_DECISION_SOURCE_FILE": str(cliq_packet),
            "ZOHO_CLI_RC_AUTONOMY_CRM_AGENT_NEXT_SOURCE_FILE": str(crm_packet),
    ...
    assert result.returncode == 0, output
    payload = json.loads(result.stdout)
    assert payload["status"] == "stop_before_operator_publish"
    assert payload["nextAction"] == "stop_for_operator_publish_execution"
    assert payload["lanes"]["openclawCliq"]["selectedPublishPath"] == (
        "local_operator_rc"
    )
    assert payload["recommendedAgentCommand"] is None
...
def test_crm_fixture_payload_preflight_blocks_template_payload(
    ...
    result = subprocess.run(
        ...
        env={
            **os.environ,
            "ZOHO_CRM_FIXTURE_PAYLOAD_FILE": str(payload_file),
            "ZOHO_CRM_FIXTURE_CLEANUP_PLAN": "delete the fixture record by fixture email after validation",
...
def test_crm_fixture_payload_preflight_requires_cleanup_selector(
    tmp_path: Path,
) -> None:
    payload_file = tmp_path / "fixture-ready.json"
    payload_file.write_text(
    ...
    raw_cleanup = "delete the test lead after validation in the CRM sandbox"
    ...
    result = subprocess.run(
        ...
        env={
            **os.environ,
            ...
            "ZOHO_CRM_FIXTURE_CLEANUP_PLAN": raw_cleanup,
            "ZOHO_CRM_FIXTURE_PREFLIGHT_REPORT_DIR": str(tmp_path / "reports"),
            "ZOHO_CRM_FIXTURE_PREFLIGHT_RUN_ID": "unit-cleanup-selector-missing",
        },
        capture_output=True,
        text=True,
        check=False,
    ...
    payload = json.loads(result.stdout)
    assert payload["status"] == "blocked"
    assert payload["blockers"] == ["cleanup_plan_selector_missing"]
...
def test_crm_fixture_operator_packet_reports_missing_payload(
    ...
    assert next_commands[0]["id"] == "provide_fixture_payload_file"
...
def test_crm_fixture_agent_next_command_summarizes_operator_input(
    tmp_path: Path,
) -> None:
    reports_dir = tmp_path / "reports"
    ...
    result = subprocess.run(
        ...
        env={
            **os.environ,
            ...
            "ZOHO_CRM_FIXTURE_AGENT_NEXT_RUN_ID": "unit-agent-next-missing",
        },
        capture_output=True,
        text=True,
        check=False,
    ...
    assert payload["status"] == "operator_input_required"
    assert payload["packet"]["commandExit"] == 1
    assert payload["packet"]["status"] == "blocked"
    assert payload["packet"]["blockers"] == [
        "cleanup_plan_missing",
        "payload_file_required",
    ]
    ...
    assert payload["reportFiles"] == {
        ...
        "operatorPacket": "crm_fixture_operator_packet_unit-agent-next-missing.json",
    }
    assert json.loads(agent_file.read_text()) == payload


def test_crm_fixture_agent_next_command_reports_agent_ready(
    tmp_path: Path,
) -> None:
        ...
        json.dumps(
            {
                ...
                "status": "blocked",
                "blockers": [],
                "nextAction": "fix_readiness_blockers",
                ...
                "operatorReview": {
                    ...
                    "agentAutomation": {
                        "nextAgentExecutableCommandId": "fix_readiness_blockers",
                        "nextAgentCommand": {
                            "id": "fix_readiness_blockers",
                            "command": (
                                "ops/scripts/crm_fixture_operator_readiness_bundle.sh"
                            ),
                            "purpose": "fix local readiness evidence",
                            "writesZohoData": False,
                            "dryRunOnly": True,
    ...
    result = subprocess.run(
        ...
        env={
            **os.environ,
            ...
            "ZOHO_CRM_FIXTURE_AGENT_NEXT_REPORT_DIR": str(reports_dir),
            "ZOHO_CRM_FIXTURE_AGENT_NEXT_FILE": str(agent_file),
            "ZOHO_CRM_FIXTURE_AGENT_NEXT_RUN_ID": "unit-agent-next-ready",
        },
        capture_output=True,
        text=True,
        check=False,
    ...
    assert payload["operatorReview"]["agentMayExecuteNextCommand"] is True
    ...
    assert payload["safety"]["normalUpsertExecuteBlocked"] is True
    assert payload["safety"]["agentMayRunNormalUpsertExecute"] is False
    assert payload["safety"]["liveFixtureExecutionBlockedForAgent"] is True
    assert payload["nextAction"] == "execute_next_agent_command"
    assert payload["reportFiles"] == {
        "agentNextCommand": "agent-next.json",
...
def test_crm_fixture_agent_next_command_stops_before_operator_write(
    tmp_path: Path,
) -> None:
    packet_file = tmp_path / "operator-packet.json"
    packet_file.write_text(
        json.dumps(
            {
                ...
                "blockers": [],
                "nextAction": "operator_review_payload_cleanup_and_approval",
                "reportFiles": {
                    "packet": "operator-packet.json",
                    "payloadPreflight": "preflight.json",
                    "dryRunReadiness": "readiness.json",
                    "smokeSummary": "summary.json",
                },
                "reportsReady": {
                    "packet": True,
                    "payloadPreflight": True,
                ...
                "operatorReview": {
                    "readyFacts": ["payload_preflight_ready"],
                    "missingFacts": [],
                    "actionBoundary": {
    ...
    result = subprocess.run(
        ...
        env={
            **os.environ,
            "ZOHO_CRM_FIXTURE_PACKET_SOURCE_FILE": str(packet_file),
            "ZOHO_CRM_FIXTURE_AGENT_NEXT_REPORT_DIR": str(tmp_path / "reports"),
            "ZOHO_CRM_FIXTURE_AGENT_NEXT_RUN_ID": "unit-agent-next-stop",
        },
        capture_output=True,
        text=True,
        check=False,
    ...
    assert str(tmp_path) not in result.stdout
    payload = json.loads(result.stdout)
    assert payload["status"] == "stop_before_operator_live_fixture"
    assert payload["packet"]["ranPacket"] is False
    assert payload["packet"]["commandExit"] == 0
    ...
    assert payload["operatorReview"]["stopReason"] == (
        "operator_only_zoho_write_or_approval_required"
    )
    assert payload["safety"]["writesZohoDataAny"] is True
    assert payload["safety"]["requiresExplicitOperatorApprovalAny"] is True
    assert payload["safety"]["operatorOnlyAny"] is True
    assert payload["nextAction"] == "stop_for_operator_live_fixture_approval"
    assert payload["reportFiles"]["operatorPacket"] == "operator-packet.json"
...
def test_crm_fixture_operator_packet_accepts_payload_preflight(
    ...
    raw_email = "fixture-ready@operator.test"
    raw_cleanup = "delete fixture by fixture email after validation"
    payload_file = tmp_path / "fixture-ready.json"
    payload_file.write_text(
        json.dumps(
            {
    ...
    next_commands = payload["operatorReview"]["nextCommands"]
    assert len(next_commands) == 1
    assert next_commands[0]["id"] == "run_crm_fixture_live_smoke_dry_run"
    assert next_commands[0]["writesZohoData"] is False
    assert next_commands[0]["dryRunOnly"] is True
    assert next_commands[0]["agentMayExecute"] is True
    assert next_commands[0]["requiresExplicitOperatorApproval"] is False
    assert "ZOHO_CRM_FIXTURE_EXECUTE=1" not in next_commands[0]["command"]
    assert "ops/scripts/crm_fixture_live_smoke.sh" in next_commands[0]["command"]
...
def test_crm_fixture_operator_packet_blocks_vague_cleanup_plan(
    ...
    next_commands = payload["operatorReview"]["nextCommands"]
    assert len(next_commands) == 1
    assert next_commands[0]["id"] == "improve_cleanup_plan"
    assert next_commands[0]["writesZohoData"] is False
    assert next_commands[0]["agentMayExecute"] is True
...
def test_crm_fixture_operator_packet_wraps_dry_run_readiness(
    ...
    result = subprocess.run(
        ...
        env={
            **os.environ,
            ...
            "ZOHO_CRM_FIXTURE_PAYLOAD_FILE": str(payload_file),
            "ZOHO_CRM_FIXTURE_CLEANUP_PLAN": "delete fixture by fixture email after validation",
    ...
    assert next_commands[0]["dryRunOnly"] is False
    ...
    assert "ZOHO_CRM_FIXTURE_EXECUTE=1" in next_commands[0]["command"]
