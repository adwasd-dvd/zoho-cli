...
8. **CRM v0.5 SDK adoption has started**: `crm-003` adds `zoho crm sdk-status`, optional `zoho-cli[crm-sdk]` packaging for official `zohocrmsdk8_0==5.0.0`, and `docs/architecture/CRM_V0_5_SDK_ADOPTION_PLAN.md`; current CRM commands still use the JSON-safe HTTP adapter by default.

## Next
1. Start `crm-004` SDK adapter skeleton while public native Cliq Bot callback verification waits on a reachable tunnel/gateway URL.
