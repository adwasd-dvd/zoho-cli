...
2. **`cliq-193` live endpoint remains externally blocked**: the latest focused rerun (`20260417_070208`) still fails app-command verification (`appCommandsError: empty_output` in `tests/auto_pilot/reports/cliq193_app_commands_probe_summary_20260417_070208.json`, CLI stderr `not_supported`), while companion status shows test-config Cliq scopes are healthy (`oauthReady: true`, `exportOauthReady: true` in `tests/auto_pilot/reports/cliq193_status_20260417_070208.json`).
3. **Latest `cliq-193` unit hardening slice is complete**: app-command list/detail normalization now also accepts mixed kebab+uppercase-tail status/mode aliases (`command-STATUS` / `action-STATUS`, `command-MODE` / `action-MODE`, plus Pascal-prefix companions) in command-row hint detection and output status mapping, with focused tests passing (`4 passed`).
