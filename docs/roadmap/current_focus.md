...
5. **Cliq modularization has started**: `cliq-210` has moved `status`/`capabilities`, `whoami`/`user-resolve`, and `users`/`teams` into smaller `zoho_cli/commands/cliq_*` modules with JSON/help parity preserved.
