...
2. **`cliq-193` live endpoint remains externally blocked**: the latest focused rerun (`20260417_074436`) still fails app-command verification (`appCommandsError: empty_output` in `tests/auto_pilot/reports/cliq193_app_commands_probe_summary_20260417_074436.json`, CLI stderr `not_supported`), while companion status shows test-config Cliq scopes are healthy (`oauthReady: true`, `exportOauthReady: true` in `tests/auto_pilot/reports/cliq193_status_20260417_074436.json`).
3. **Latest `cliq-193` unit hardening slice is complete**: app-command list/detail normalization now also accepts Pascal-prefix mixed snake+uppercase-tail mode aliases (`Command_MODE` / `Action_MODE`) in command-row hint detection and output status mapping, with focused tests passing (`2 passed`).
