...
2. **`cliq-193` live endpoint remains externally blocked**: the latest focused rerun (`20260417_062954`) still fails app-command verification (`appCommandsError: empty_output` in `tests/auto_pilot/reports/cliq193_app_commands_probe_summary_20260417_062954.json`, CLI stderr `not_supported`), while companion status shows test-config Cliq scopes are healthy (`oauthReady: true`, `exportOauthReady: true` in `tests/auto_pilot/reports/cliq193_status_20260417_062954.json`).
