...
- Controlled live outbound smoke, local inbound polling dry-runs, Bot webhook receive/auth/normalize smoke, status/read lifecycle smoke, turn-ledger loop-prevention smoke, status/routing diagnostics smoke, AI troubleshooting dry-runs, fake host native dispatch smoke, live smoke gate, trusted reply evidence checking with `sha256:` sender/message/reply references, and host compatibility revalidation are now unblocked for trusted targets; a public Bot callback has been verified through an operator tunnel, and rollout now waits on controlled trusted Mention-to-agent reply evidence plus a durable tunnel/gateway decision. Template: `docs/releases/OPENCLAW_CLIQ_TRUSTED_REPLY_EVIDENCE_TEMPLATE.json`.
...
- Next active target: complete the trusted Zoho Cliq Bot Mention -> OpenClaw `zoho-employee-test` Codex reply smoke and validate its redacted artifact with `ops/scripts/openclaw_cliq_trusted_reply_evidence.sh`, then run the operator-approved CRM fixture live run only after a dedicated payload and cleanup plan exist.
...
2. Keep remaining Cliq helper-heavy modularization opportunistic while the
   channel lane moves.
3. Complete the native OpenClaw Cliq channel live reply smoke by verifying a trusted Bot Mention routes to the intended Codex-backed agent and emits exactly one Cliq reply, then require `trusted_reply_recorded` from `ops/scripts/openclaw_cliq_trusted_reply_evidence.sh`.
