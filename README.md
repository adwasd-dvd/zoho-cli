...
- OAuth scope presets: `zoho login --with-crm` keeps the lightweight CRM default (`ZohoCRM.modules.ALL`, `ZohoCRM.settings.ALL`); `zoho login --with-storepilot-crm` adds StorePilot bootstrap scopes for users, org, bulk, notifications, and COQL. Validate with `zoho crm status --scope-profile storepilot --check-auth`.
