...
7. **OpenClaw channel inbound polling is normalized and deduped**: `cliq-channel-401` added the installable package, `cliq-channel-402` expanded config/SecretRef setup, `cliq-channel-416` added setup wizard UX, `cliq-channel-403` added secure policy gates, `cliq-channel-414` added SDK session/mention/approval seams, `cliq-channel-404` added JSON-safe `zoho` process execution, `cliq-channel-405` wired native outbound delivery, and `cliq-channel-406` added fixture-backed polling normalization/dedupe over `zoho cliq chats` + `zoho cliq context`.

## Next
1. Start `cliq-channel-407`: implement webhook inbound normalization and verification against the same event shape as polling.
...
- **Native OpenClaw Cliq channel v0.4**: config/setup/security/SDK/CLI adapter/outbound smoke and local inbound polling dry-runs are ready; webhook intake, turn ledger loop prevention, and observability remain before production bidirectional testing.
