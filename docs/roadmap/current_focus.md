...
8. **CRM v0.5 SDK adoption is in v8 alignment**: `crm-003` adds `zoho crm sdk-status`, optional `zoho-cli[crm-sdk]` packaging for official `zohocrmsdk8_0==5.0.0`, and `docs/architecture/CRM_V0_5_SDK_ADOPTION_PLAN.md`; `crm-004` adds `zoho_cli/crm_sdk.py` with data-center mapping, `ZOHO_CRM_SDK_RESOURCE_PATH` cache/resource policy, and JSON-safe read-only adapter skeleton; `crm-005` adds explicit `--adapter sdk-v8` read-only gates for modules/fields/list/get/search. Current CRM commands still use the HTTP adapter by default.

## Next
1. Start `crm-006` v8 REST alignment decision while public native Cliq Bot callback verification waits on a reachable tunnel/gateway URL.
