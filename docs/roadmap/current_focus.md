...
8. **CRM v0.5 SDK adoption is moving to write audit persistence**: `crm-003` adds `zoho crm sdk-status`, optional `zoho-cli[crm-sdk]` packaging for official `zohocrmsdk8_0==5.0.0`, and `docs/architecture/CRM_V0_5_SDK_ADOPTION_PLAN.md`; `crm-004` adds `zoho_cli/crm_sdk.py` with data-center mapping, `ZOHO_CRM_SDK_RESOURCE_PATH` cache/resource policy, and JSON-safe read-only adapter skeleton; `crm-005` adds explicit `--adapter sdk-v8` read-only gates for modules/fields/list/get/search; `crm-006` locks HTTP v2 as default and SDK/API v8 as explicit-only; `crm-007` adds `zoho crm write-plan` and `writeSurfacePolicy` with `writesEnabled=false`; `crm-008` adds `zoho crm upsert` as dry-run-only with payload digest/audit output and `--execute` blocked; `crm-009` adds `zoho crm upsert-gate` and keeps live upsert deferred until audit persistence and controlled live fixture evidence exist. Current CRM reads still use the HTTP adapter by default.

## Next
1. Start `crm-010` CRM write audit persistence while public native Cliq Bot callback verification waits on a reachable tunnel/gateway URL.
