...
3. **Latest `cliq-193` unit hardening slice is complete**: app-command list/detail normalization now also accepts mixed kebab+camel displayname aliases (`command-displayName`/`action-displayName`), with focused tests passing (`2 passed`).
