...
| Source drift check | `OPENCLAW_CLIQ_SOURCE_DRIFT_RUN_ID=20260512T135144Z-crm-state-sync-postcommit-source ops/scripts/openclaw_cliq_rc_source_drift_check.sh` passed after the CRM Lane 1 state-sync commit with `status=package_source_unchanged`, no blockers, `headCommit=0319d9b8ebabf532f0b76230991a1feae2ee78a1`, `headMatchesManifestSource=false`, `repoChangedSinceManifest=true`, `repoChangedFileCount=29`, `packageDrift.packageChangedSinceManifest=false`, and no dirty package files. Any package file drift still blocks with `package_source_drift_detected` before operator publish. |
