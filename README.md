...
| CRM | 🚧 In progress | v0_5_write_surface_planning | Read-only scaffold is present; `crm-003/004/005/006` added SDK readiness diagnostics, optional SDK packaging, the default-disabled SDK adapter, explicit `--adapter sdk-v8` read-only gates, and the HTTP v2 vs SDK/API v8 policy while preserving the current JSON-safe HTTP adapter as default. |
...
- Next active target: CRM v0.5 write-surface planning while public native Cliq Bot callback verification waits on a reachable tunnel/gateway URL.
...
4. Plan CRM write-surface safety gates without enabling writes before explicit approval, scopes, confirmation, and audit output are locked.
